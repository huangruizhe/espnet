...
class Nbest(object):
    ...
    def __init__(self, fsa: k2.Fsa, shape: k2.RaggedShape, path_to_seq_map, token_ids) -> None:
        """
        Args:
          fsa:
        ...
        self.fsa = fsa
        self.shape = shape
        self.path_to_seq_map = path_to_seq_map
    ...
    def from_lattice(
        ...
        unique_word_seqs, _, new2old = word_seq.unique(
        ...
        token_seqs = k2.ragged.index(lattice.labels.contiguous(), path)
        ...
        return Nbest(fsa=fsa, shape=utt_to_path_shape, path_to_seq_map=path_to_seq_map, token_ids=token_ids)
    ...
    def intersect(self, lattice: k2.Fsa, use_double_scores=True) -> "Nbest":
        ...
        return Nbest(fsa=one_best, shape=self.shape, path_to_seq_map=self.path_to_seq_map, token_ids=self.token_ids)
...
class k2Speech2Text:
    ...
    def __init__(
        ...
        if not hasattr(self.decode_graph, "lm_scores"):
            self.decode_graph.lm_scores = self.decode_graph.scores.clone()
    ...
    @torch.no_grad()
    def __call__(
        self, keys, batch: Dict[str, Union[torch.Tensor, np.ndarray]]
        ...
        logging.info(f"logp_encoder_output info: {logp_encoder_output.shape}")
        ...
        logging.info("Lattices info:")
        logging.info(f"search_beam_size={self.search_beam_size},  output_beam_size={self.output_beam_size}, min_active_states={self.min_active_states}, max_active_states={self.max_active_states}")
        logging.info(f"{lattices.shape} #state:{lattices[0].shape} #arcs:{lattices[0].num_arcs}")

        restrict_lattices_size = False
        if restrict_lattices_size:
            sbeam_size = self.search_beam_size
            obeam_size = self.output_beam_size
            max_states = self.max_active_states
            is_done = False
            # Note: this part may affect the WER and oracle WER!
            while (lattices[0].num_arcs > 600000 and lattices[0].shape[0] > 150000) or \
                lattices[0].num_arcs > 1200000 or lattices[0].shape[0] > 250000 or \
                    not is_done:
                sbeam_size = max(10, int(sbeam_size * 0.9))
                obeam_size = max(10, int(obeam_size * 0.9))
                max_states = int(max_states * 0.94)
                logging.info(f"Handling this utterance: reduce lattice size sbeam_size={sbeam_size}, obeam_size={obeam_size}, max_states={max_states}")
                try:
                    lattices = k2.intersect_dense_pruned(
                        self.decode_graph,
                        dense_fsa_vec,
                    ...
                    logging.info("Lattices info:")
                    logging.info(f"search_beam_size={sbeam_size},  output_beam_size={obeam_size}, min_active_states={self.min_active_states}, max_active_states={max_states}")
                    logging.info(f"{lattices.shape} #state:{lattices[0].shape} #arcs:{lattices[0].num_arcs}")
                    is_done = True
                except:
                    logging.info("K2 failed: need to redo this utterance...")
                    is_done = False

            # if keys[0] == "en_4829_0A_00118": # or keys[0] == "fsh_111905_exA_B_100000_105825":
            #     logging.info(f"special handling uid={keys[0]}")
            #     lattices = k2.intersect_dense_pruned(
            #         self.decode_graph,
            #         dense_fsa_vec,
            #         18,
            ...
            #     )
            #     logging.info("Lattices info:")
            #     logging.info(f"search_beam_size={18},  output_beam_size={18}, min_active_states={self.min_active_states}, max_active_states={3500}")
            #     logging.info(f"{lattices.shape} #state:{lattices[0].shape} #arcs:{lattices[0].num_arcs}")
        ...
        if self.use_nbest_rescoring:
            my_num_paths = self.num_paths
            if lattices[0].num_arcs > 1860000 and lattices[0].shape[0] > 179000:
                logging.info(f"Handling this utterance: set num_paths from {self.num_paths} to half")
                my_num_paths = int(self.num_paths / 2)
            ...
            (
                ...
                lattices, my_num_paths, self.device, self.nbest_batch_size
            )

            logging.info(f"am_scores.shape={am_scores.shape}")
            ...
            my_num_paths = max(self.num_paths, self.nbest * 2)
            # if lattices[0].num_arcs > 1860000 and lattices[0].shape[0] > 179000 or \
            #    lattices[0].num_arcs > 1550000 and lattices[0].shape[0] > 327000 or \
            #    lattices[0].num_arcs > 1720000 and lattices[0].shape[0] > 282000 or \
            #     lattices[0].num_arcs > 608000 and lattices[0].shape[0] > 214000:
            #     logging.info(f"Handling this utterance: set num_paths from {self.num_paths} to half")
            #     my_num_paths = int(self.num_paths / 2)
            
            nbest = Nbest.from_lattice(
                lattice=lattices,
                # num_paths=max(self.num_paths, self.nbest * 10),  # TODO: Do we need this?
                num_paths=my_num_paths,
                use_double_scores=True,
                nbest_scale=self.nbest_scale,
                # nbest_scale=1.0,
            )

            nbest = nbest.intersect(lattices)  # TODO: this function is confusing to me. It actually returns one best, but ...
            tot_scores2 = nbest.tot_scores()
            am_scores2 = nbest.compute_am_scores().values
            # lm_scores = nbest.compute_lm_scores()

            token_ids2 = nbest.token_ids

            logging.info(f"am_scores.shape={am_scores2.shape}, nbest_scale={self.nbest_scale}")
            ...
            encoder_out = enc.index_select(0, nbest.path_to_seq_map.to(torch.long)).to(
                self.device
            )  # [batch, T, dim]
            encoder_out_lens = encoder_out_lens.index_select(
                0, nbest.path_to_seq_map.to(torch.long)
            ).to(
                self.device
            )  # [batch]
            ...
            if self.search_params:
                scores = [
                    (am_s, decoder_s, nnlm_s) for am_s, decoder_s, nnlm_s in zip(
                        am_scores.tolist(),
                        decoder_scores.tolist(),
                        nnlm_scores.tolist()
                    )
                ]
                best_paths = k2.index_fsa(nbest.fsa, torch.as_tensor(np.arange(len(scores)), dtype=torch.int32))
                ...
                topk = batch_tot_scores.tolist()
                topk = torch.as_tensor(topk).topk(min(abs(self.nbest), len(topk)))
                topk_scores = topk.values.tolist()
                topk_indices = topk.indices.type(torch.int32)
                best_paths = k2.index_fsa(nbest.fsa, topk_indices)
            ...
            nbest = Nbest.from_lattice(
                lattice=lattices,
                # num_paths=max(self.num_paths, self.nbest * 10),  # TODO: Do we need this?
                num_paths=max(self.num_paths, self.nbest * 2),
                use_double_scores=True,
                nbest_scale=self.nbest_scale, # 0.8,
                # nbest_scale=1.0,
            )
        ...
        if best_paths is not None:
            best_path_aux_labels = dict()
            best_path_aux_labels["word"] = best_paths[0].aux_labels.tolist()   # word labels
            best_path_aux_labels["token"] = best_paths[0].tokens.tolist()       # token labels
            # best_path_aux_labels = self.converter.ids2tokens(best_path_aux_labels)
            best_path_aux_labels["word"] =  [str(lbl) for lbl in best_path_aux_labels["word"]]
            best_path_aux_labels["token"] =  [str(lbl) for lbl in best_path_aux_labels["token"]]
        else:
            best_path_aux_labels = None
...
def inference(
    ...
    logging.info("speech2text_kwargs=" + str(speech2text_kwargs))
    ...
    with DatadirWriter(output_dir) as writer:
        ...
        for batch_idx, (keys, batch) in enumerate(loader):
            logging.info(f"batch_idx={batch_idx}: {keys}")
            # if keys[0] != 'sw_4577-A_006965-007620':
            # if keys[0] != 'en_4156-A_030763-031116':
            ...
            # if keys[0] == 'sw_4507-B_007979-008744' or keys[0] == 'sw_4689-B_009014-010379':
            #     results = [("", [], [], 0)]
            #     best_path_aux_labels = None
            # else:
            try:
                results, best_path_aux_labels = speech2text(keys, batch)
            except:
                logging.info(f"{keys[0]} has encountered errors during decoding, hyp=\"\".")
                results = [("", [], [], 0)]
                best_path_aux_labels = None
            ...
            if speech2text.search_params:
                for n, (text, token, token_int, score) in zip(range(1, len(results) + 1), results):
                    # Create a directory: outdir/{n}best_recog
                    ibest_writer = writer[f"nbest_recog"]
                    key = f"{keys[0]}-{n}"
                    ...
                    ibest_writer["score"][key] = ' '.join(map(str, score))
                ...
                key = keys[0]
                for n, (text, token, token_int, score) in zip(range(1, abs(nbest) + 1), results):
                    # Create a directory: outdir/{n}best_recog
                ...
                if best_path_aux_labels is not None:
                    best_writer = writer["1best_recog"]
                    best_writer["best_path_aux_labels.words"][key] = " ".join(best_path_aux_labels["word"])
                    best_writer["best_path_aux_labels.tokens"][key] = " ".join(best_path_aux_labels["token"])
                else:
                    best_writer = writer["1best_recog"]
                    best_writer["best_path_aux_labels.words"][key] = ""
                    best_writer["best_path_aux_labels.tokens"][key] = ""
...
def get_parser():
    ...
    group.add_argument("--search_params", type=str2bool, default=False)
