...
class k2Speech2Text:
    ...
    def load_or_build_decode_graph(
        ...
        if config.get("graph", None) is not None:
            ...
            # 3. L graph for TL
            if config.get("L", None) is not None:
            ...
            # 4. G graph
            if config.get("G", None) is not None:
                with open(config["G"], "r") as f:
                    ...
                    G = k2.remove_epsilon(G)   # TODO: uncomment these three lines will cause some errors in k2
                    G = k2.connect(G)
                    G = k2.arc_sort(G)
            ...
            if L is not None and G is None:
            ...
            elif L is not None and G is not None:
        ...
        if config.get("graph_save_path", None) is not None:
